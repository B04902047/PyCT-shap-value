...
class Concolic:
    # named __init2__ to be called "manually"
    def __init2__(self, value, expr=None, engine= None):
        from libct.solver import Solver
        from libct.utils import py2smt
        self.engine = engine if engine is not None else Solver._expr_has_engines_and_equals_value(
            expr, value)
...
class MetaFinal(type):
    def __new__(cls, name, bases, classdict):
        for b in bases:
            if isinstance(b, cls):
                raise TypeError(
                    f"type '{b.__name__}' is not an acceptable base type")
