...
def _int(obj):
    ...
    if isinstance(obj, Concolic) and hasattr(obj, '__int2__'):
        return obj.__int2__()
    return int(obj)
...
def _str(obj):
    from libct.concolic import Concolic
    if isinstance(obj, Concolic) and hasattr(obj, '__str2__'):
        return obj.__str2__()
    return str(obj)
...
def _is(obj1, obj2):
    ...
    if obj1 is obj2:
        return True
    if isinstance(obj1, Concolic):
        obj1 = unwrap(obj1)
    if isinstance(obj2, Concolic):
        obj2 = unwrap(obj2)
    return obj1 is obj2


def ConcolicObject(value, expr=None, engine: ExplorationEngine | None = None):
    ...
    if type(value) is bool:
        return ConcolicBool(value, expr, engine)
    if type(value) is float:
        return ConcolicFloat(value, expr, engine)
    if type(value) is int:
        return ConcolicInt(value, expr, engine)
    if type(value) is str:
        return ConcolicStr(value, expr, engine)
    # TODO: Are there other types of mutable sequences? What about "slice"?
    if isinstance(value, list):
        return list(map(ConcolicObject, value))
    return value


# call primitive's casting function to avoid getting stuck when the concolic's function is modified.
def unwrap(x):
    from libct.concolic.bool import ConcolicBool
    from libct.concolic.float import ConcolicFloat
    from libct.concolic.int import ConcolicInt
    ...
    if type(x) is ConcolicFloat:
        return float.__float__(x)
    if type(x) is ConcolicInt:
        return int.__int__(x)
    if type(x) is ConcolicStr:
...
def py2smt(x):  # convert the Python object into the smtlib2 string constant
    ...
    if type(x) is (float):
        return '(- ' + f"{-x:.15f}" + ')' if x < 0 else f"{x:.15f}"
    if type(x) is str:
        x = x.replace("\\", "\\\\").replace("\r", "\\r").replace(
            "\n", "\\n").replace("\t", "\\t").replace('"', '""')
        # this kind of encoding is just a workaround but incorrect since it changes the length of "\r", "\n", "\t".
        x_new = ""
        for ch in x:
...
def get_module_from_rootdir_and_modpath(rootdir, modpath):
    ...
    spec = importlib.util.spec_from_file_location(
        modpath, os.path.abspath(filepath))
